          ...
          web3.eth.getCoinbase(function(err, data) {
                         if (err == null) {
                              ...
                              web3.eth.getBalance(coinbase, function(err, data) {
                                   if (err == null) {
                                        coinbaseBalance = data;
                                        $("#coinbase").text(coinbase);
                                        $("#coinbaseBalance").text(web3.fromWei(coinbaseBalance, "ether") + " ETH");
                                   }
                              });
                         }
                    });
...
function displayExperience(experienceId, startDate, endDate, employer, city, country, description) {
     ...
     // description
     var pMarker = document.createElement("p");
     pMarker.innerHTML = web3.toAscii(description);
     divMarker.appendChild(pMarker);
     ...
     pMarker.className = "text-muted";
     var spanMarker = document.createElement("span");
     spanMarker.className = "glyphicon glyphicon-map-marker";
     pMarker.appendChild(spanMarker);
